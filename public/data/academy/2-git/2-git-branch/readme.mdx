...
Tambien facilitan el trabajo en grupo, cada persona puede hacer su tarea en una rama separda. 🔀

Lo facil que es cambiar entre ramas, hace que git sea tan popular. 🤩
...
> ### HEAD
> #### Es un puntero especial. Es donde estamos parados, la version del repositorio que estamos viendo!
...
```
El puntero `HEAD` se movio commit apuntado por la rama `testing`.
...
Las ramas devergieron, ya no compoarten la misma historia!
...
Cuando una tarea esta lista, esa rama se fuciona con la rama base o principal.
...
1. Vamos a `master` con `git checkout mastar`
1. Hacemos cambios y creamos un commit `git add . && git commit -m "C5"`

Por ultimo, hacemos el _merge commit_ pasandole un mensaje con `-m`:
```
...
`git log --graph` nos muestra la historia, senalando en el grafico cada _commit_ con un `*`:
...
Para recrear esta situacion, supongamos que en la rama `master` tambien habiamos creado un archivo `LICENCE`.
...
Si abrimos el archivo `LICENCE` vamos a ver algo asi:
...
* `=======`: separa los cambios de la rama actual (`HEAD`) de lso cambios que viene de la rama que estmaos queriendo mergear (`iss53`)
* `>>>>>>> iss53`: en donde termina el conflicto

#### Nuestra tarea es:

1. Edidar el archivo `LICENCE` y decidir con que cambios quedarse. Las opciones son:
...
1. Eliminar los marcadores para que git sepa que no hay mas conflicto
1. Prepear el archivo
1. Hacer el commit
...
Git ya sabe que todos los conflictos fueron solucionados, pero nos dice que el merge todavia esta activo.
...
De esta forma se creo el _merge commit_ con los conflictos solucionados! 🎉
...
- Ver el grafico de commits con `git log`

#### 2. Merge

- Fusionar distintas ramas en `master`
- Crear un conflicto y solucioanrlo!
...
Merge: combinar la historia de `<nombre>` con la rama actual dandole un mensaje para el commit
```
git merge <nombre> -m "mensaje del merge commit"
```

Ver la historia del repositorio como un grafico
```
git log --graph
```
